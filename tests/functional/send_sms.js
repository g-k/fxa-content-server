 ...
 * file, You can obtain one at http://mozilla.org/MPL/2.0/. */
 ...
 define([
   ...
   registerSuite({
     ...
     'valid phone number': function () {
       return this.remote
        .then(openPage(SEND_SMS_URL, SELECTOR_SEND_SMS_HEADER))
        .then(type(SELECTOR_SEND_SMS_PHONE_NUMBER, '1234567890'))
        .then(click(SELECTOR_SEND_SMS_SUBMIT))
        .then(testElementExists(SELECTOR_SMS_SENT_HEADER))
        .then(testElementTextInclude(SELECTOR_SMS_SENT_TO, '123-456-7890'));
     },

     'valid phone number (contains spaces)': function () {
       return this.remote
        .then(openPage(SEND_SMS_URL, SELECTOR_SEND_SMS_HEADER))
        .then(type(SELECTOR_SEND_SMS_PHONE_NUMBER, '  123 456 7890  '))
        .then(click(SELECTOR_SEND_SMS_SUBMIT))
        .then(testElementExists(SELECTOR_SMS_SENT_HEADER))
        .then(testElementTextInclude(SELECTOR_SMS_SENT_TO, '123-456-7890'));
     },

     'valid phone number (contains punctuation)': function () {
       return this.remote
        .then(openPage(SEND_SMS_URL, SELECTOR_SEND_SMS_HEADER))
        .then(type(SELECTOR_SEND_SMS_PHONE_NUMBER, '123.456,78-90'))
        .then(click(SELECTOR_SEND_SMS_SUBMIT))
        .then(testElementExists(SELECTOR_SMS_SENT_HEADER))
        .then(testElementTextInclude(SELECTOR_SMS_SENT_TO, '123-456-7890'));
     },

     'valid phone number, error sending': function () {
     }
   });
 });
