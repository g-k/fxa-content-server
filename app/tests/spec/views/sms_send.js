     ...
     describe('submit', () => {
       describe('succeeds', () => {
         it('it delegates to `account.sendSms`, calls `_onSendSmsSuccess` with the phone number', () => {
           ...
           view.$('input[type=tel]').val('1234');

           return view.submit()
             .then(() => {
               assert.isTrue(account.sendSms.calledOnce);
               assert.isTrue(account.sendSms.calledWith('1234', SmsMessageIds.FIREFOX_MOBILE_INSTALL));
               assert.isTrue(view._onSendSmsSuccess.calledOnce);
               assert.isTrue(view._onSendSmsSuccess.calledWith('1234'));
             });
         });
       });
